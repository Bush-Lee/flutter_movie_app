...
import 'package:movie_app/ui/general/listView/listView.dart';
import 'package:movie_app/ui/general/video/video.dart';
...
class BaseState extends State<Base> {
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> _screens = [Home(), Home(), Home()];
    int _index;
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: TabBar(
            tabs: _tabTitles.map((value) => Padding(
              padding: const EdgeInsets.all(8.0),
              child: Text(value, style: TextStyle(fontSize: 15),),
            )).toList(),
            onTap: (index) {_index = index;}, // use setState
          ),
        ),
        body: TabBarView(
